/**
 * Minimal 6×8 bitmap font covering printable ASCII (0x20–0x7E).
 * Each character is 6 pixels wide × 8 pixels tall.
 * Stored as 8 bytes per glyph — each byte is one row (top→bottom),
 * and each bit (MSB first, 6 bits used) is one pixel (1 = on, 0 = off).
 */
...
/** Returns the 8-row bitmap for a character */
export function getGlyph(ch: string): number[] {
  return GLYPHS[ch] ?? FALLBACK
}

/** Rasterize a string into a pixel buffer (0=bg, 1=fg).
 *  Returns { pixels: Uint8Array, width, height } where width = text.length * CHAR_W + 2*padX
 */
export function rasterizeText(
  ...
  const width = text.length * CHAR_W + padX * 2
  const height = CHAR_H + padY * 2
  const pixels = new Uint8Array(width * height) // all 0 = bg

  for (let ci = 0; ci < text.length; ci++) {
    const glyph = getGlyph(text[ci])
    const xOff = padX + ci * CHAR_W
    for (let row = 0; row < CHAR_H; row++) {
      const byte = glyph[row]
      for (let bit = 0; bit < CHAR_W; bit++) {
        // MSB first, we use the top 6 bits (shift right by 2 to align 6 bits into 8-bit byte)
        const on = (byte >> (7 - bit)) & 1
        const px = (padY + row) * width + xOff + bit
        pixels[px] = on ? 1 : 0
      }
    }
  }
